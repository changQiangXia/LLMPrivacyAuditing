...
1. **直接抽取是否成功**。左侧第一列几乎全部贴近 `0`，说明在当前设置下，没有观察到完整 canary 被直接补全出来的证据。
2. **exposure 是否抬升**。第二列区分度最高，是当前最有信息量的一列。
3. **MIA 是否同步增强**。第三列和第四列有变化，但整体仍停留在弱信号区间。
...
按当前数据规模、模型大小和评估口径，**实验没有观察到模型把 canary 直接完整背出来，但已经观察到模型对 canary 的统计记忆倾向增强；这种倾向在训练更久、学习率更高、重复次数更多时会更明显。**

理解：

- **“没有直接完整背出来”**。这里说的是 canary 抽取攻击结果。实验里把 canary 的前缀交给模型继续补全，没有出现“模型把整条 canary 原样补全出来”的情况，所以完整抽取成功率是 `0`。
- **“统计记忆倾向增强”**。这里不是说模型已经把内容一字不差背出，而是说模型对这些字符串已经变得更“熟”。最直接的证据是：`lora_standard` 的 `avg_exposure = 1.651849 ± 0.019638`，高于 `lora_no_canary = 1.261029 ± 0.008580` 和 `base_model = 1.259471`。这表示模型虽然没有直接复述完整 canary，但已经更容易偏向这些目标字符串。
- **“训练越强，记忆越明显”**。这里对应的是消融实验里的趋势。比如 `epochs=1 -> 2 -> 3` 时，`avg_exposure` 从 `1.660983` 升到 `2.039426`、`3.055287`；`canary_repeats=1 -> 3 -> 5` 时，`avg_exposure` 从 `1.399037` 升到 `1.561922`、`1.660983`。这表示训练时间更长、重复更多、更新更激进时，模型更容易把这些字符串记住。

即“模型已经开始记住这些内容，而且训练越强，记住的倾向越明显”。

### 8.2 展开解释

- **直接泄露证据较弱**：正式 baseline 与当前 ablation 中，完整 canary 抽取成功率均为 `0`。这表示当前没有看到模型直接生成完整 canary 的现象。
- **统计记忆信号明确存在**：`lora_standard` 的 `avg_exposure = 1.651849 ± 0.019638`，明显高于 `lora_no_canary = 1.261029 ± 0.008580` 与 `base_model = 1.259471`。这表示模型虽然没有直接背出内容，但已经对目标字符串表现出更强的记忆倾向。
- **MIA 只能提供辅助证据**：多数 `MIA AUC` 位于 `0.50` 到 `0.54` 区间，低误报阈值下的 `TPR@FPR=1e-3` 多数接近 `0`。这表示当前 member 与 nonmember 的可分离性存在轻微信号，但还谈不上强证据。
- **训练强度是当前最敏感的风险放大因素**：`epochs=1 -> 2 -> 3` 时，`avg_exposure` 从 `1.660983` 升至 `2.039426`、`3.055287`；`lr=1e-4 -> 2e-4 -> 5e-4` 时，`avg_exposure` 从 `1.570356` 升至 `1.660983`、`1.786848`。这表示训练更久、更新更激进时，模型更容易记住目标字符串。
- **dedup 在当前设置下影响较小**：`lora_dedup = 1.664285` 与 `lora_standard = 1.651849 ± 0.019638` 接近，说明当前数据中的精确重复不是主要风险来源。
...
- 当前最强结论来自 **exposure**，不是来自 canary 直接抽取成功率。
- 当前 MIA 属于 **baseline MIA**，信号整体偏弱，不宜单独承担主结论。
- 多数 ablation 仍为 **单次运行结果**，当前更适合用于趋势判断与敏感性定位。
- `decode_safe` 属于 **推理阶段防护**，不应表述为训练阶段隐私保护。
